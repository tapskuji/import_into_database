...
$data = __DIR__ . '/unprocessed/students.txt';

function processFlatFile($file) {
    $handle = fopen($file, 'r');
...
foreach (processFlatFile($data) as $row) {
    try {
        $db->beginTransaction();
...
if ($hasErrors) {
    $newName = __DIR__ . '/incomplete/' . (new DateTime())->format('Ymd_His') . '.txt';
} else {
    $newName = __DIR__ . '/processed/' . (new DateTime())->format('Ymd_His') . '.txt';
}
