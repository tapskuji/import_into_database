...
$file = __DIR__ . '/unprocessed/students.xlsx';
...
foreach (processXlsx($file) as $row) {
    try {
        $db->beginTransaction();
...
if ($hasErrors) {
    $newName = __DIR__ . '/incomplete/' . (new DateTime())->format('Ymd_His') . '.xlsx';
} else {
    $newName = __DIR__ . '/processed/' . (new DateTime())->format('Ymd_His') . '.xlsx';
}
